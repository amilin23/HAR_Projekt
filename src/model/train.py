...
def _split_data(X: np.ndarray, y: np.ndarray, g: np.ndarray, seed: int):
    # Keep your original approach but make it unambiguous with a separator
...
def _save_classes(path: str, classes: List[str]) -> None:
    # One label per line => avoids CSV index/header parsing bugs
...
def main():
    ...
    Xtr, Xval, Xte, ytr, yval, yte = _split_data(X, y, g, seed=cfg.random_state)
    ...
    model.save(os.path.join(cfg.out_dir, "ax6_two_wrist_lstm.keras"))
    np.save(os.path.join(cfg.out_dir, "norm_mean.npy"), mean)
    np.save(os.path.join(cfg.out_dir, "norm_std.npy"), std)
    _save_classes(os.path.join(cfg.out_dir, "classes.csv"), classes)
